                ...
                try
                    ...
                    | AddTweet(username,request_id,tweet,request) ->    timer.Restart()
                                                                        ...
                                                                        if count % 10000L = 0L then
                                                                            printfn "Average Tweet processing after %i tweets is %A microseconds and total %A milliseconds" count (timetaken/(ticksPerMicroSecond*(count|>float))) (timetaken/ticksPerMilliSecond) 
        ...
        try
            ...
            | GetTags(username,request)->   timer.Restart()
                                            ...
                                            if gettagscnt%100 = 0 then
                                                    printfn "Average time taken to process get username_mentions is %A microseconds after %i getmentions Requests and total %A milliseconds" ((gettagstime|>float)/(ticksPerMicroSecond*(gettagscnt|>float))) gettagscnt ((gettagstime|>float)/ticksPerMilliSecond)
            ...
            let rec loop() =
                actor {
                    ...
                    if request_id%10000 = 0 then
                        printfn "Time taken to handle %i requests from clients is %A milliseconds" request_id ((ticks|>float)/ticksPerMilliSecond)
